...
void loop()
{
  ...
  case DET: // 차량이 오기를 대기하는 상태(2초간 threshold 보다 가깝게 있어야 측정됨)
    ...
    { // threshold와 거리를 비교
      Serial.println(cnt);
      if (cnt >= 5)
      { // 2초 동안 측정되면 REC stat으로 넘어감 & car/detect 토픽에 인식되었음을 알림
        cnt = 0;
  ...
  case ESC:
    if (getDistance(20) >= threshold)
    {
      Serial.println(cnt);
      if (cnt >= 10)
      { // 5초 동안 측정되면 차량 차단기 초기화와 함께, DET stat으로 넘어간다.
        cnt = 0;
        stat = DET;
        reset_servo();
        break;
